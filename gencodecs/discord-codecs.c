...
#include <stdlib.h>
#include "carray.h"
static char *_gc_strndup(const char *src, size_t len) {
...
static void discord_activity_emoji_cleanup(struct discord_activity_emoji *this);

static void discord_activity_party_init(struct discord_activity_party *this);
static void discord_activity_party_cleanup(struct discord_activity_party *this);
...
static size_t discord_role_tag_from_jsmnf(jsmnf *root, const char buf[],
                                          struct discord_role_tag *this);

static size_t
discord_audit_log_entry_from_jsmnf(jsmnf *root, const char buf[],
                                   struct discord_audit_log_entry *this);

static size_t
discord_audit_log_entries_from_jsmnf(jsmnf *root, const char buf[],
...
static size_t
discord_audit_log_change_from_jsmnf(jsmnf *root, const char buf[],
                                    struct discord_audit_log_change *this);

static size_t
discord_audit_log_changes_from_jsmnf(jsmnf *root, const char buf[],
...
static size_t
discord_invite_metadata_from_jsmnf(jsmnf *root, const char buf[],
...
static size_t
discord_message_activity_from_jsmnf(jsmnf *root, const char buf[],
                                    struct discord_message_activity *this);

static size_t
discord_message_reference_from_jsmnf(jsmnf *root, const char buf[],
                                     struct discord_message_reference *this);

static size_t discord_reaction_from_jsmnf(jsmnf *root, const char buf[],
                                          struct discord_reaction *this);

static size_t discord_reactions_from_jsmnf(jsmnf *root, const char buf[],
                                           struct discord_reactions *this);

static size_t discord_overwrite_from_jsmnf(jsmnf *root, const char buf[],
                                           struct discord_overwrite *this);

static size_t discord_overwrites_from_jsmnf(jsmnf *root, const char buf[],
                                            struct discord_overwrites *this);

static size_t
discord_thread_metadata_from_jsmnf(jsmnf *root, const char buf[],
                                   struct discord_thread_metadata *this);

static size_t
discord_thread_member_from_jsmnf(jsmnf *root, const char buf[],
                                 struct discord_thread_member *this);

static size_t discord_attachment_from_jsmnf(jsmnf *root, const char buf[],
                                            struct discord_attachment *this);

static size_t
discord_channel_mention_from_jsmnf(jsmnf *root, const char buf[],
                                   struct discord_channel_mention *this);

static size_t
discord_allowed_mention_from_jsmnf(jsmnf *root, const char buf[],
...
static size_t
discord_get_guild_widget_from_jsmnf(jsmnf *root, const char buf[],
                                    struct discord_get_guild_widget *this);

static size_t discord_integration_from_jsmnf(jsmnf *root, const char buf[],
                                             struct discord_integration *this);

static size_t
discord_integrations_from_jsmnf(jsmnf *root, const char buf[],
...
static size_t
discord_list_guild_members_from_jsmnf(jsmnf *root, const char buf[],
...
static size_t
discord_stage_instances_from_jsmnf(jsmnf *root, const char buf[],
                                   struct discord_stage_instances *this);

static size_t discord_stickers_from_jsmnf(jsmnf *root, const char buf[],
                                          struct discord_stickers *this);

static size_t
discord_sticker_item_from_jsmnf(jsmnf *root, const char buf[],
                                struct discord_sticker_item *this);

static size_t
discord_sticker_items_from_jsmnf(jsmnf *root, const char buf[],
                                 struct discord_sticker_items *this);

static size_t
discord_sticker_pack_from_jsmnf(jsmnf *root, const char buf[],
...
static size_t discord_connection_from_jsmnf(jsmnf *root, const char buf[],
...
static size_t
discord_voice_region_from_jsmnf(jsmnf *root, const char buf[],
...
static size_t
discord_presence_updates_from_jsmnf(jsmnf *root, const char buf[],
                                    struct discord_presence_updates *this);

static size_t discord_activity_from_jsmnf(jsmnf *root, const char buf[],
                                          struct discord_activity *this);

static size_t discord_activities_from_jsmnf(jsmnf *root, const char buf[],
...
static size_t
discord_activity_emoji_from_jsmnf(jsmnf *root, const char buf[],
                                  struct discord_activity_emoji *this);

static size_t
discord_activity_party_from_jsmnf(jsmnf *root, const char buf[],
                                  struct discord_activity_party *this);

static size_t
discord_activity_assets_from_jsmnf(jsmnf *root, const char buf[],
                                   struct discord_activity_assets *this);

static size_t
discord_activity_secrets_from_jsmnf(jsmnf *root, const char buf[],
                                    struct discord_activity_secrets *this);

static size_t
discord_activity_button_from_jsmnf(jsmnf *root, const char buf[],
                                   struct discord_activity_button *this);

static size_t
discord_activity_buttons_from_jsmnf(jsmnf *root, const char buf[],
                                    struct discord_activity_buttons *this);

static size_t discord_team_member_from_jsmnf(jsmnf *root, const char buf[],
                                             struct discord_team_member *this);

static size_t
discord_team_members_from_jsmnf(jsmnf *root, const char buf[],
...
static size_t
discord_select_option_from_jsmnf(jsmnf *root, const char buf[],
                                 struct discord_select_option *this);

static size_t
discord_interaction_data_from_jsmnf(jsmnf *root, const char buf[],
                                    struct discord_interaction_data *this);

static size_t
discord_resolved_data_from_jsmnf(jsmnf *root, const char buf[],
...
static jsonbcode discord_role_tag_to_jsonb(jsonb *b, char buf[], size_t size,
                                           const struct discord_role_tag *this);

static jsonbcode
discord_audit_log_entry_to_jsonb(jsonb *b, char buf[], size_t size,
...
static jsonbcode
discord_audit_log_change_to_jsonb(jsonb *b, char buf[], size_t size,
...
static jsonbcode
discord_invite_metadata_to_jsonb(jsonb *b, char buf[], size_t size,
...
static jsonbcode
discord_message_activity_to_jsonb(jsonb *b, char buf[], size_t size,
...
static jsonbcode discord_reaction_to_jsonb(jsonb *b, char buf[], size_t size,
                                           const struct discord_reaction *this);

static jsonbcode
discord_reactions_to_jsonb(jsonb *b, char buf[], size_t size,
                           const struct discord_reactions *this);

static jsonbcode
discord_overwrite_to_jsonb(jsonb *b, char buf[], size_t size,
                           const struct discord_overwrite *this);

static jsonbcode
discord_overwrites_to_jsonb(jsonb *b, char buf[], size_t size,
                            const struct discord_overwrites *this);

static jsonbcode
discord_thread_metadata_to_jsonb(jsonb *b, char buf[], size_t size,
                                 const struct discord_thread_metadata *this);

static jsonbcode
discord_thread_member_to_jsonb(jsonb *b, char buf[], size_t size,
                               const struct discord_thread_member *this);

static jsonbcode
discord_attachment_to_jsonb(jsonb *b, char buf[], size_t size,
                            const struct discord_attachment *this);

static jsonbcode
discord_channel_mention_to_jsonb(jsonb *b, char buf[], size_t size,
                                 const struct discord_channel_mention *this);

static jsonbcode
discord_allowed_mention_to_jsonb(jsonb *b, char buf[], size_t size,
...
static jsonbcode
discord_get_guild_widget_to_jsonb(jsonb *b, char buf[], size_t size,
                                  const struct discord_get_guild_widget *this);

static jsonbcode
discord_integration_to_jsonb(jsonb *b, char buf[], size_t size,
                             const struct discord_integration *this);

static jsonbcode
discord_integrations_to_jsonb(jsonb *b, char buf[], size_t size,
...
static jsonbcode
discord_stage_instances_to_jsonb(jsonb *b, char buf[], size_t size,
                                 const struct discord_stage_instances *this);

static jsonbcode discord_stickers_to_jsonb(jsonb *b, char buf[], size_t size,
                                           const struct discord_stickers *this);

static jsonbcode
discord_sticker_item_to_jsonb(jsonb *b, char buf[], size_t size,
                              const struct discord_sticker_item *this);

static jsonbcode
discord_sticker_items_to_jsonb(jsonb *b, char buf[], size_t size,
                               const struct discord_sticker_items *this);

static jsonbcode
discord_sticker_pack_to_jsonb(jsonb *b, char buf[], size_t size,
...
static jsonbcode
discord_connection_to_jsonb(jsonb *b, char buf[], size_t size,
...
static jsonbcode
discord_voice_region_to_jsonb(jsonb *b, char buf[], size_t size,
...
static jsonbcode
discord_presence_updates_to_jsonb(jsonb *b, char buf[], size_t size,
                                  const struct discord_presence_updates *this);

static jsonbcode discord_activity_to_jsonb(jsonb *b, char buf[], size_t size,
                                           const struct discord_activity *this);

static jsonbcode
discord_activities_to_jsonb(jsonb *b, char buf[], size_t size,
...
static jsonbcode
discord_activity_emoji_to_jsonb(jsonb *b, char buf[], size_t size,
                                const struct discord_activity_emoji *this);

static jsonbcode
discord_activity_party_to_jsonb(jsonb *b, char buf[], size_t size,
                                const struct discord_activity_party *this);

static jsonbcode
discord_activity_assets_to_jsonb(jsonb *b, char buf[], size_t size,
                                 const struct discord_activity_assets *this);

static jsonbcode
discord_activity_secrets_to_jsonb(jsonb *b, char buf[], size_t size,
                                  const struct discord_activity_secrets *this);

static jsonbcode
discord_activity_button_to_jsonb(jsonb *b, char buf[], size_t size,
                                 const struct discord_activity_button *this);

static jsonbcode
discord_activity_buttons_to_jsonb(jsonb *b, char buf[], size_t size,
                                  const struct discord_activity_buttons *this);

static jsonbcode
discord_team_member_to_jsonb(jsonb *b, char buf[], size_t size,
                             const struct discord_team_member *this);

static jsonbcode
discord_team_members_to_jsonb(jsonb *b, char buf[], size_t size,
...
static jsonbcode
discord_select_option_to_jsonb(jsonb *b, char buf[], size_t size,
                               const struct discord_select_option *this);

static jsonbcode
discord_interaction_data_to_jsonb(jsonb *b, char buf[], size_t size,
                                  const struct discord_interaction_data *this);

static jsonbcode
discord_resolved_data_to_jsonb(jsonb *b, char buf[], size_t size,
...
void discord_application_cleanup(struct discord_application *this) {
  ...
  if (this->terms_of_service_url)
    free(this->terms_of_service_url);

  if (this->privacy_policy_url)
    free(this->privacy_policy_url);
...
static void discord_optional_audit_entry_info_cleanup(
  ...
  if (this->delete_member_days)
    free(this->delete_member_days);

  if (this->members_removed)
    free(this->members_removed);
...
static void discord_optional_audit_entry_infos_cleanup(
    struct discord_optional_audit_entry_infos *this) {
  __carray_free(this, struct discord_optional_audit_entry_info, NULL,
...
void discord_embed_author_cleanup(struct discord_embed_author *this) {
  ...
  if (this->proxy_icon_url)
    free(this->proxy_icon_url);
}
...
void discord_thread_response_body_cleanup(
    struct discord_thread_response_body *this) {
...
void discord_get_channel_messages_cleanup(
    struct discord_get_channel_messages *this) {
...
void discord_bulk_delete_messages_cleanup(
    struct discord_bulk_delete_messages *this) {
...
discord_get_guild_widget_cleanup(struct discord_get_guild_widget *this) {
  ...
  if (this->instant_invite)
    free(this->instant_invite);
...
void discord_create_guild_channel_cleanup(
    struct discord_create_guild_channel *this) {
...
void discord_modify_guild_sticker_cleanup(
    struct discord_modify_guild_sticker *this) {
...
static void
discord_get_webhook_message_cleanup(struct discord_get_webhook_message *this) {}
void discord_edit_webhook_message_cleanup(
    struct discord_edit_webhook_message *this) {
...
discord_activity_assets_cleanup(struct discord_activity_assets *this) {
  ...
  if (this->large_image)
    free(this->large_image);

  if (this->large_text)
    free(this->large_text);

  if (this->small_image)
    free(this->small_image);

  if (this->small_text)
    free(this->small_text);
}
static void
discord_activity_secrets_cleanup(struct discord_activity_secrets *this) {
...
void discord_application_commands_cleanup(
    struct discord_application_commands *this) {
...
static void discord_application_command_option_choices_cleanup(
    struct discord_application_command_option_choices *this) {
  __carray_free(
      this, struct discord_application_command_option_choice, NULL,
      discord_application_command_option_choice_cleanup(&__CARRAY_OPERAND_A));
}
static void discord_application_command_interaction_data_option_cleanup(
    struct discord_application_command_interaction_data_option *this) {
  ...
  if (this->options) {
    discord_application_command_interaction_data_options_cleanup(this->options);
    free(this->options);
  };
...
void discord_guild_application_command_permissions_cleanup(
  ...
  __carray_free(this, struct discord_guild_application_command_permission, NULL,
...
void discord_interaction_cleanup(struct discord_interaction *this) {
  ...
  if (this->guild_locale)
    free(this->guild_locale);
}
static void
discord_interaction_data_cleanup(struct discord_interaction_data *this) {
...
void discord_interaction_response_cleanup(
    struct discord_interaction_response *this) {
...
size_t strings_from_jsmnf(jsmnf *root, const char buf[], struct strings *this) {
  ...
  HASH_ITER(hh, root->child, f, tmp) {
    ...
    if (f && f->val->type == JSMN_STRING)
      ret +=
          jsmnf_unescape(&o, buf + f->val->start, f->val->end - f->val->start);
...
size_t snowflakes_from_jsmnf(jsmnf *root, const char buf[],
  ...
  HASH_ITER(hh, root->child, f, tmp) {
    u64snowflake o;
    if (f)
      sscanf(buf + f->val->start, "%" SCNu64, &o);
    carray_insert(this, f->idx, o);
  }
  return ret;
}
...
size_t discord_role_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->id);
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->tags + discord_role_tag_from_jsmnf(f, buf, this->tags);
  };
...
static size_t discord_role_tag_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->bot_id);

  f = jsmnf_find(root, "integration_id", sizeof("integration_id") - 1);
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->integration_id);
...
size_t discord_application_from_jsmnf(jsmnf *root, const char buf[],
  ...
  f = jsmnf_find(root, "privacy_policy_url", sizeof("privacy_policy_url") - 1);
  if (f && f->val->type == JSMN_STRING)
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->owner + discord_user_from_jsmnf(f, buf, this->owner);
  };
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->team + discord_team_from_jsmnf(f, buf, this->team);
  };

  f = jsmnf_find(root, "guild_id", sizeof("guild_id") - 1);
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->guild_id);

  f = jsmnf_find(root, "primary_sku_id", sizeof("primary_sku_id") - 1);
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->primary_sku_id);
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->flags);
  return ret;
}
...
size_t discord_audit_log_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret +=
        sizeof *this->audit_log_entries +
        discord_audit_log_entries_from_jsmnf(f, buf, this->audit_log_entries);
  };
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->users + discord_users_from_jsmnf(f, buf, this->users);
  };
...
discord_audit_log_entry_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->target_id);
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->user_id);
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->options +
           discord_optional_audit_entry_infos_from_jsmnf(f, buf, this->options);
  };
...
static size_t discord_optional_audit_entry_info_from_jsmnf(
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->channel_id);
  ...
  f = jsmnf_find(root, "delete_member_days", sizeof("delete_member_days") - 1);
  if (f && f->val->type == JSMN_STRING)
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->message_id);
...
static size_t discord_get_guild_audit_log_from_jsmnf(
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->before);
...
size_t discord_invite_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->guild + discord_guild_from_jsmnf(f, buf, this->guild);
  };
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret +=
        sizeof *this->inviter + discord_user_from_jsmnf(f, buf, this->inviter);
  };
  ...
  f = jsmnf_find(root, "target_application", sizeof("target_application") - 1);
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    this->target_application = calloc(1, sizeof *this->target_application);
    ret += sizeof *this->target_application +
  ...
  if (f && f->val->type == JSMN_PRIMITIVE)
    this->approximate_member_count = (int)strtol(buf + f->val->start, NULL, 10);

  f = jsmnf_find(root, "expires_at", sizeof("expires_at") - 1);
  if (f && f->val->type == JSMN_PRIMITIVE)
    cog_iso8601_to_unix_ms(buf + f->val->start, f->val->end - f->val->start,
                           &this->expires_at);
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret +=
        sizeof *this->stage_instance +
        discord_invite_stage_instance_from_jsmnf(f, buf, this->stage_instance);
  };
...
size_t discord_get_invite_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->guild_scheduled_event_id);
...
size_t discord_channel_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->permission_overwrites +
           discord_overwrites_from_jsmnf(f, buf, this->permission_overwrites);
  };
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->last_message_id);
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->owner_id);

  f = jsmnf_find(root, "application_id", sizeof("application_id") - 1);
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->application_id);

  f = jsmnf_find(root, "parent_id", sizeof("parent_id") - 1);
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->parent_id);

  f = jsmnf_find(root, "last_pin_timestamp", sizeof("last_pin_timestamp") - 1);
  if (f && f->val->type == JSMN_PRIMITIVE)
    cog_iso8601_to_unix_ms(buf + f->val->start, f->val->end - f->val->start,
  ...
  f = jsmnf_find(root, "voice_quality_mode", sizeof("voice_quality_mode") - 1);
  if (f && f->val->type == JSMN_PRIMITIVE)
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->thread_metadata +
           discord_thread_metadata_from_jsmnf(f, buf, this->thread_metadata);
  };
...
size_t discord_message_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->author + discord_user_from_jsmnf(f, buf, this->author);
  };
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret +=
        sizeof *this->embeds + discord_embeds_from_jsmnf(f, buf, this->embeds);
  };
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->webhook_id);
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret +=
        sizeof *this->message_reference +
        discord_message_reference_from_jsmnf(f, buf, this->message_reference);
  };
  ...
  f = jsmnf_find(root, "referenced_message", sizeof("referenced_message") - 1);
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->interaction +
           discord_message_interaction_from_jsmnf(f, buf, this->interaction);
  };
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret +=
        sizeof *this->thread + discord_channel_from_jsmnf(f, buf, this->thread);
  };
...
discord_message_reference_from_jsmnf(jsmnf *root, const char buf[],
  ...
  f = jsmnf_find(root, "fail_if_not_exists", sizeof("fail_if_not_exists") - 1);
  if (f && f->val->type == JSMN_PRIMITIVE)
...
static size_t discord_reaction_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->emoji + discord_emoji_from_jsmnf(f, buf, this->emoji);
  };
  return ret;
}
static size_t discord_reactions_from_jsmnf(jsmnf *root, const char buf[],
...
static size_t discord_overwrite_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->allow);

  f = jsmnf_find(root, "deny", sizeof("deny") - 1);
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->deny);
  return ret;
}
static size_t discord_overwrites_from_jsmnf(jsmnf *root, const char buf[],
                                            struct discord_overwrites *this) {
...
discord_thread_metadata_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f && f->val->type == JSMN_PRIMITIVE)
    this->auto_archive_duration = (int)strtol(buf + f->val->start, NULL, 10);
...
size_t
discord_embed_thumbnail_from_jsmnf(jsmnf *root, const char buf[],
...
discord_allowed_mention_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->roles + snowflakes_from_jsmnf(f, buf, this->roles);
  };

  f = jsmnf_find(root, "users", sizeof("users") - 1);
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    this->users = calloc(1, sizeof *this->users);
    ret += sizeof *this->users + snowflakes_from_jsmnf(f, buf, this->users);
  };
...
size_t discord_modify_channel_from_jsmnf(jsmnf *root, const char buf[],
  ...
  f = jsmnf_find(root, "video_quality_mode", sizeof("video_quality_mode") - 1);
  if (f && f->val->type == JSMN_PRIMITIVE)
...
size_t discord_get_channel_messages_from_jsmnf(
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->around);
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->after);
...
size_t discord_create_message_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->allowed_mentions +
           discord_allowed_mention_from_jsmnf(f, buf, this->allowed_mentions);
  };
...
size_t discord_bulk_delete_messages_from_jsmnf(
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret +=
        sizeof *this->messages + snowflakes_from_jsmnf(f, buf, this->messages);
  };
  return ret;
}
...
size_t discord_create_channel_invite_from_jsmnf(
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->target_user_id);
...
size_t discord_follow_news_channel_from_jsmnf(
  ...
  f = jsmnf_find(root, "webhook_channel_id", sizeof("webhook_channel_id") - 1);
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->webhook_channel_id);
...
size_t discord_emoji_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    this->roles = calloc(1, sizeof *this->roles);
    ret += sizeof *this->roles + discord_roles_from_jsmnf(f, buf, this->roles);
  };

  f = jsmnf_find(root, "user", sizeof("user") - 1);
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    this->user = calloc(1, sizeof *this->user);
    ret += sizeof *this->user + discord_user_from_jsmnf(f, buf, this->user);
  };
...
size_t discord_guild_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->afk_channel_id);
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->widget_channel_id);

  f = jsmnf_find(root, "verification_level", sizeof("verification_level") - 1);
  if (f && f->val->type == JSMN_PRIMITIVE)
  ...
  if (f && f->val->type == JSMN_PRIMITIVE)
    this->explicit_content_filter = (int)strtol(buf + f->val->start, NULL, 10);
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret +=
        sizeof *this->emojis + discord_emojis_from_jsmnf(f, buf, this->emojis);
  };

  f = jsmnf_find(root, "features", sizeof("features") - 1);
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    this->features = calloc(1, sizeof *this->features);
    ret += sizeof *this->features + strings_from_jsmnf(f, buf, this->features);
  };
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->system_channel_id);

  f = jsmnf_find(root, "system_channel_flags",
                 sizeof("system_channel_flags") - 1);
  if (f && f->val->type == JSMN_PRIMITIVE)
    this->system_channel_flags = (int)strtol(buf + f->val->start, NULL, 10);

  f = jsmnf_find(root, "rules_channel_id", sizeof("rules_channel_id") - 1);
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->rules_channel_id);
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->public_updates_channel_id);

  f = jsmnf_find(root, "max_video_channel_users",
                 sizeof("max_video_channel_users") - 1);
  if (f && f->val->type == JSMN_PRIMITIVE)
    this->max_video_channel_users = (int)strtol(buf + f->val->start, NULL, 10);
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->welcome_screen +
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->stage_instances +
           discord_stage_instances_from_jsmnf(f, buf, this->stage_instances);
  };
...
discord_get_guild_widget_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    this->members = calloc(1, sizeof *this->members);
    ret +=
        sizeof *this->members + discord_users_from_jsmnf(f, buf, this->members);
  };
...
static size_t discord_integration_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->role_id);
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    this->application = calloc(1, sizeof *this->application);
    ret +=
        sizeof *this->application +
        discord_integration_application_from_jsmnf(f, buf, this->application);
  };
  return ret;
}
static size_t
...
size_t discord_welcome_screen_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->welcome_channels +
           discord_welcome_screen_channels_from_jsmnf(f, buf,
                                                      this->welcome_channels);
  };
...
static size_t discord_welcome_screen_channel_from_jsmnf(
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->emoji_id);
...
discord_create_guild_role_from_jsmnf(jsmnf *root, const char buf[],
  ...
  f = jsmnf_find(root, "permissions", sizeof("permissions") - 1);
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->permissions);
...
size_t discord_guild_scheduled_event_from_jsmnf(
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->creator_id);
  ...
  f = jsmnf_find(root, "scheduled_end_time", sizeof("scheduled_end_time") - 1);
  if (f && f->val->type == JSMN_PRIMITIVE)
    cog_iso8601_to_unix_ms(buf + f->val->start, f->val->end - f->val->start,
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->entity_id);
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret +=
        sizeof *this->creator + discord_user_from_jsmnf(f, buf, this->creator);
  };
...
size_t discord_guild_template_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->source_guild_id);
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->serialized_source_guild +
...
size_t discord_sticker_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->pack_id);
...
discord_sticker_pack_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->sku_id);

  f = jsmnf_find(root, "cover_sticker_id", sizeof("cover_sticker_id") - 1);
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->cover_sticker_id);
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->banner_asset_id);
...
static size_t discord_create_guild_sticker_from_jsmnf(
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret +=
        sizeof *this->file + discord_attachment_from_jsmnf(f, buf, this->file);
  };
  return ret;
}
...
size_t discord_user_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->public_flags);
  return ret;
}
...
size_t discord_create_dm_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->recipient_id);

  return ret;
}
size_t
discord_create_group_dm_from_jsmnf(jsmnf *root, const char buf[],
...
size_t
discord_execute_webhook_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->thread_id);
...
size_t
discord_presence_update_from_jsmnf(jsmnf *root, const char buf[],
...
size_t discord_team_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->owner_user_id);
  return ret;
}
static size_t discord_team_member_from_jsmnf(jsmnf *root, const char buf[],
                                             struct discord_team_member *this) {
  ...
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->team_id);
...
size_t discord_application_command_from_jsmnf(
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret +=
        sizeof *this->options +
        discord_application_command_options_from_jsmnf(f, buf, this->options);
  };

  f = jsmnf_find(root, "default_permission", sizeof("default_permission") - 1);
  if (f && f->val->type == JSMN_PRIMITIVE)
    this->default_permission = ('t' == buf[f->val->start]);

  f = jsmnf_find(root, "version", sizeof("version") - 1);
  if (f)
    sscanf(buf + f->val->start, "%" SCNu64, &this->version);
  return ret;
}
...
static size_t discord_application_command_option_from_jsmnf(
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    ...
    ret += sizeof *this->choices +
           discord_application_command_option_choices_from_jsmnf(f, buf,
                                                                 this->choices);
  };
...
static size_t discord_application_command_option_choice_from_jsmnf(
  ...
  f = jsmnf_find(root, "value", sizeof("value") - 1);
  if (f) {
    this->value = _gc_strndup(buf + f->val->start, f->val->end - f->val->start);
    ret += f->val->end - f->val->start;
  };
  return ret;
}
static size_t discord_application_command_option_choices_from_jsmnf(
  ...
  if (!nelems)
    return 0;
  ret = sizeof *this * nelems;
  __carray_init(this, nelems, struct discord_application_command_option_choice,
                , );
  HASH_ITER(hh, root->child, f, tmp) {
    struct discord_application_command_option_choice o = {0};
    ret += discord_application_command_option_choice_from_jsmnf(f, buf, &o);
    carray_insert(this, f->idx, o);
  }
  return ret;
}
...
size_t discord_guild_application_command_permissions_from_jsmnf(
  ...
  HASH_ITER(hh, root->child, f, tmp) {
    ...
    ret += discord_guild_application_command_permission_from_jsmnf(f, buf, &o);
    carray_insert(this, f->idx, o);
  }
  return ret;
}
...
discord_resolved_data_from_jsmnf(jsmnf *root, const char buf[],
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    this->members = calloc(1, sizeof *this->members);
    ret += sizeof *this->members + snowflakes_from_jsmnf(f, buf, this->members);
  };
  ...
  if (f && (f->val->type == JSMN_OBJECT || f->val->type == JSMN_ARRAY)) {
    this->channels = calloc(1, sizeof *this->channels);
    ret +=
        sizeof *this->channels + snowflakes_from_jsmnf(f, buf, this->channels);
  };
...
size_t strings_from_json(const char buf[], size_t size, struct strings *this) {
  ...
  if (ret >= 0)
    ret = strings_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t json_values_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = json_values_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t snowflakes_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = snowflakes_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}

size_t integers_from_json(const char buf[], size_t size,
                          struct integers *this) {
  jsmnf *root = jsmnf_init();
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = integers_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_role_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_role_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_roles_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_roles_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_application_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_application_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_audit_log_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_audit_log_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_invite_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_invite_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_invites_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_invites_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_get_invite_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_get_invite_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_channel_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_channel_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_channels_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_channels_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_message_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_message_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_messages_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_messages_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
discord_followed_channel_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_followed_channel_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_thread_members_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_thread_members_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_attachments_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_attachments_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_embed_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_embed_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_embeds_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_embeds_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_embed_thumbnail_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_embed_thumbnail_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_embed_video_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_embed_video_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_embed_image_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_embed_image_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_embed_provider_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_embed_provider_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_embed_author_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_embed_author_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_embed_footer_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_embed_footer_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_embed_field_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_embed_field_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_embed_fields_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_embed_fields_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_modify_channel_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_modify_channel_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_create_message_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_create_message_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_get_reactions_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_get_reactions_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_edit_message_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_edit_message_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_follow_news_channel_from_json(
    const char buf[], size_t size, struct discord_follow_news_channel *this) {
  jsmnf *root = jsmnf_init();
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
...
size_t discord_list_active_threads_from_json(
    const char buf[], size_t size, struct discord_list_active_threads *this) {
  jsmnf *root = jsmnf_init();
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
...
size_t discord_emoji_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_emoji_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_emojis_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_emojis_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
discord_create_guild_emoji_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_create_guild_emoji_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
discord_modify_guild_emoji_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_modify_guild_emoji_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_guild_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_guild_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_guilds_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_guilds_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_guild_preview_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_guild_preview_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_guild_member_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_guild_member_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_guild_members_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_guild_members_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_ban_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_ban_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_bans_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_bans_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_welcome_screen_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_welcome_screen_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_create_guild_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_create_guild_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_modify_guild_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_modify_guild_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_modify_guild_channel_positions_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_modify_guild_channel_positions_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
discord_add_guild_member_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_add_guild_member_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}

size_t discord_modify_guild_member_from_json(
    const char buf[], size_t size, struct discord_modify_guild_member *this) {
  jsmnf *root = jsmnf_init();
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
...
discord_create_guild_ban_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_create_guild_ban_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
discord_create_guild_role_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_create_guild_role_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
discord_modify_guild_role_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_modify_guild_role_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
discord_begin_guild_prune_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_begin_guild_prune_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_modify_current_user_voice_state_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_modify_current_user_voice_state_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_list_scheduled_events_for_guild_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_list_scheduled_events_for_guild_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_get_guild_scheduled_event_users_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_get_guild_scheduled_event_users_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_guild_template_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_guild_template_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_create_guild_from_guild_template_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_create_guild_from_guild_template_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_stage_instance_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_stage_instance_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_sticker_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_sticker_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_user_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_user_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_users_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_users_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_connections_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_connections_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}

size_t discord_modify_current_user_from_json(
    const char buf[], size_t size, struct discord_modify_current_user *this) {
  jsmnf *root = jsmnf_init();
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
...
size_t discord_create_dm_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_create_dm_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_create_group_dm_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_create_group_dm_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_voice_state_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_voice_state_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_voice_states_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_voice_states_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_voice_regions_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_voice_regions_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_webhook_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_webhook_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_webhooks_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_webhooks_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_create_webhook_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_create_webhook_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_modify_webhook_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_modify_webhook_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_execute_webhook_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_execute_webhook_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_identify_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_identify_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
discord_voice_state_status_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_voice_state_status_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_presence_update_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_presence_update_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}

size_t discord_session_start_limit_from_json(
    const char buf[], size_t size, struct discord_session_start_limit *this) {
  jsmnf *root = jsmnf_init();
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
...
size_t discord_team_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_team_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}

size_t discord_application_command_from_json(
    const char buf[], size_t size, struct discord_application_command *this) {
  jsmnf *root = jsmnf_init();
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
...
size_t discord_guild_application_command_permissions_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_guild_application_command_permissions_from_jsmnf(root, buf,
                                                                   this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_application_command_permission_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_application_command_permission_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_application_command_permissions_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_application_command_permissions_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_create_global_application_command_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_create_global_application_command_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_edit_global_application_command_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_edit_global_application_command_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_create_guild_application_command_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_create_guild_application_command_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_edit_guild_application_command_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_edit_guild_application_command_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_edit_application_command_permissions_from_json(
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_edit_application_command_permissions_from_jsmnf(root, buf,
                                                                  this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_component_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_component_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_components_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_components_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_select_options_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_select_options_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_interaction_from_json(const char buf[], size_t size,
  ...
  int ret = jsmnf_start(root, buf, size);
  if (ret >= 0)
    ret = discord_interaction_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
size_t discord_edit_original_interaction_response_from_json(
  ...
  if (ret >= 0)
    ret =
        discord_edit_original_interaction_response_from_jsmnf(root, buf, this);
  jsmnf_cleanup(root);
  return ret;
}
...
jsonbcode strings_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  jsonbcode code;
  if (0 > (code = jsonb_array(b, buf, size)))
    return code;
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i) {
      if (0 >
          (code = jsonb_string(b, buf, size, this->array[i],
                               this->array[i] ? strlen(this->array[i]) : 0)))
        return code;
    }
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
  return code;
}
...
jsonbcode discord_role_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  jsonbcode code;
  if (0 > (code = jsonb_object(b, buf, size)))
    return code;
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "hoist", sizeof("hoist") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->hoist)))
      return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "position", sizeof("position") - 1)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "permissions",
      ...
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->permissions,
                                 this->permissions ? strlen(this->permissions)
                                                   : 0)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "managed", sizeof("managed") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->managed)))
      return code;
    ...
    if (this->tags != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "tags", sizeof("tags") - 1)))
        return code;
      if (0 > (code = discord_role_tag_to_jsonb(b, buf, size, this->tags)))
        return code;
    }
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
  return code;
}
jsonbcode discord_roles_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_role_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
static jsonbcode
discord_role_tag_to_jsonb(jsonb *b, char buf[], size_t size,
                          const struct discord_role_tag *this) {
  jsonbcode code;
  if (0 > (code = jsonb_object(b, buf, size)))
    return code;
  if (this != NULL) {
    ...
    if (this->bot_id != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "bot_id", sizeof("bot_id") - 1)))
        return code;
      else {
        char tok[64];
        int toklen;
...
jsonbcode discord_application_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "description",
      ...
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->description,
                                 this->description ? strlen(this->description)
                                                   : 0)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "bot_require_code_grant",
                              sizeof("bot_require_code_grant") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->bot_require_code_grant)))
      return code;
    ...
    if (0 >
        (code = jsonb_string(
             b, buf, size, this->privacy_policy_url,
             this->privacy_policy_url ? strlen(this->privacy_policy_url) : 0)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "summary", sizeof("summary") - 1)))
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->summary,
    ...
    if (0 > (code = jsonb_key(b, buf, size, "verify_key",
      ...
      return code;
    if (0 >
        (code = jsonb_string(b, buf, size, this->verify_key,
                             this->verify_key ? strlen(this->verify_key) : 0)))
      return code;

    if (this->team != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "team", sizeof("team") - 1)))
        return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "guild_id", sizeof("guild_id") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "cover_image",
      ...
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->cover_image,
                                 this->cover_image ? strlen(this->cover_image)
                                                   : 0)))
      return code;
...
jsonbcode discord_audit_log_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (this->threads != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "threads", sizeof("threads") - 1)))
        return code;
      if (0 > (code = discord_channels_to_jsonb(b, buf, size, this->threads)))
        return code;
    }
    ...
    if (this->webhooks != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "webhooks", sizeof("webhooks") - 1)))
        return code;
      if (0 > (code = discord_webhooks_to_jsonb(b, buf, size, this->webhooks)))
        return code;
    }
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
...
discord_audit_log_entry_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "target_id", sizeof("target_id") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
    ...
    if (this->changes != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "changes", sizeof("changes") - 1)))
        return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "user_id", sizeof("user_id") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
    ...
    if (this->options != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "options", sizeof("options") - 1)))
        return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "reason", sizeof("reason") - 1)))
      return code;
...
static jsonbcode discord_optional_audit_entry_info_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_string(
             b, buf, size, this->delete_member_days,
             this->delete_member_days ? strlen(this->delete_member_days) : 0)))
      return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "role_name", sizeof("role_name") - 1)))
      return code;
    if (0 >
        (code = jsonb_string(b, buf, size, this->role_name,
                             this->role_name ? strlen(this->role_name) : 0)))
      return code;
...
discord_audit_log_change_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "new_value", sizeof("new_value") - 1)))
      return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "old_value", sizeof("old_value") - 1)))
      return code;
...
static jsonbcode discord_audit_log_changes_to_jsonb(
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_audit_log_change_to_jsonb(b, buf, size,
                                                        &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
...
static jsonbcode discord_get_guild_audit_log_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "before", sizeof("before") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
...
jsonbcode discord_invite_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (this->guild != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "guild", sizeof("guild") - 1)))
        return code;
      if (0 > (code = discord_guild_to_jsonb(b, buf, size, this->guild)))
        return code;
    }

    if (0 > (code = jsonb_key(b, buf, size, "channel", sizeof("channel") - 1)))
      return code;
    ...
    if (this->inviter != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "inviter", sizeof("inviter") - 1)))
        return code;
      if (0 > (code = discord_user_to_jsonb(b, buf, size, this->inviter)))
        return code;
    }
    ...
    if (this->target_user != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "target_user",
        ...
        return code;
      if (0 > (code = discord_user_to_jsonb(b, buf, size, this->target_user)))
        return code;
    }
    ...
    if (0 > (code = jsonb_key(b, buf, size, "approximate_presence_count",
      ...
      return code;
    if (0 >
        (code = jsonb_number(b, buf, size, this->approximate_presence_count)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "approximate_member_count",
                              sizeof("approximate_member_count") - 1)))
      return code;
    if (0 > (code = jsonb_number(b, buf, size, this->approximate_member_count)))
      return code;
    ...
    if (this->expires_at != 0) {
        ...
        return code;
      {
        char tok[64];
        int toklen;
        toklen = cog_unix_ms_to_iso8601(tok, sizeof(tok), this->expires_at);
        if (0 > (code = jsonb_string(b, buf, size, tok, toklen)))
          return code;
      };
    }
...
jsonbcode discord_invites_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_invite_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
discord_invite_metadata_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "uses", sizeof("uses") - 1)))
      return code;
    if (0 > (code = jsonb_number(b, buf, size, this->uses)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "max_uses", sizeof("max_uses") - 1)))
      return code;
    if (0 > (code = jsonb_number(b, buf, size, this->max_uses)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "max_age", sizeof("max_age") - 1)))
      return code;
    if (0 > (code = jsonb_number(b, buf, size, this->max_age)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "temporary", sizeof("temporary") - 1)))
      return code;
    ...
    if (this->created_at != 0) {
        ...
        return code;
      {
        char tok[64];
        int toklen;
        toklen = cog_unix_ms_to_iso8601(tok, sizeof(tok), this->created_at);
        if (0 > (code = jsonb_string(b, buf, size, tok, toklen)))
          return code;
      };
    }
...
static jsonbcode discord_invite_stage_instance_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (this->members != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "members", sizeof("members") - 1)))
        return code;
      if (0 >
          (code = discord_guild_members_to_jsonb(b, buf, size, this->members)))
        return code;
    }
  ...
  if (this != NULL) {
    ...
    if (this->guild_scheduled_event_id != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "guild_scheduled_event_id",
        ...
        int toklen;
        toklen =
            sprintf(tok, "\"%" PRIu64 "\"", this->guild_scheduled_event_id);
        if (0 > (code = jsonb_token(b, buf, size, tok, toklen)))
          return code;
      }
    }
...
jsonbcode discord_channel_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "permission_overwrites",
                              sizeof("permission_overwrites") - 1)))
      return code;
    if (0 > (code = discord_overwrites_to_jsonb(b, buf, size,
                                                this->permission_overwrites)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "nsfw", sizeof("nsfw") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->nsfw)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "bitrate", sizeof("bitrate") - 1)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "recipients",
                              sizeof("recipients") - 1)))
      return code;
    if (0 > (code = discord_users_to_jsonb(b, buf, size, this->recipients)))
      return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "owner_id", sizeof("owner_id") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "parent_id", sizeof("parent_id") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
      ...
      return code;
    {
      ...
      int toklen;
      toklen =
          cog_unix_ms_to_iso8601(tok, sizeof(tok), this->last_pin_timestamp);
      if (0 > (code = jsonb_string(b, buf, size, tok, toklen)))
        return code;
    };
    ...
    if (0 > (code = jsonb_key(b, buf, size, "rtc_region",
      ...
      return code;
    if (0 >
        (code = jsonb_string(b, buf, size, this->rtc_region,
    ...
    if (0 > (code = jsonb_key(b, buf, size, "thread_metadata",
      ...
      return code;
    if (0 > (code = discord_thread_metadata_to_jsonb(b, buf, size,
                                                     this->thread_metadata)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "member", sizeof("member") - 1)))
      return code;
    if (0 > (code = discord_thread_member_to_jsonb(b, buf, size, this->member)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "default_auto_archive_duration",
                              sizeof("default_auto_archive_duration") - 1)))
      return code;
...
jsonbcode discord_channels_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_channel_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
jsonbcode discord_message_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "author", sizeof("author") - 1)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "member", sizeof("member") - 1)))
      return code;
    if (0 > (code = discord_guild_member_to_jsonb(b, buf, size, this->member)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "content", sizeof("content") - 1)))
      return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "timestamp", sizeof("timestamp") - 1)))
      return code;
    {
      char tok[64];
      int toklen;
    ...
    if (this->edited_timestamp != 0) {
        ...
        return code;
      {
        ...
        int toklen;
        toklen =
            cog_unix_ms_to_iso8601(tok, sizeof(tok), this->edited_timestamp);
        if (0 > (code = jsonb_string(b, buf, size, tok, toklen)))
          return code;
      };
    }
    ...
    if (0 > (code = jsonb_key(b, buf, size, "tts", sizeof("tts") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->tts)))
      return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "mentions", sizeof("mentions") - 1)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "mention_roles",
      ...
      return code;
    if (0 > (code = discord_roles_to_jsonb(b, buf, size, this->mention_roles)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "attachments",
      ...
      return code;
    if (0 >
        (code = discord_attachments_to_jsonb(b, buf, size, this->attachments)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "embeds", sizeof("embeds") - 1)))
      return code;
    if (0 > (code = discord_embeds_to_jsonb(b, buf, size, this->embeds)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "reactions", sizeof("reactions") - 1)))
      return code;
    if (0 > (code = discord_reactions_to_jsonb(b, buf, size, this->reactions)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "pinned", sizeof("pinned") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->pinned)))
      return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "activity", sizeof("activity") - 1)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "application",
      ...
      return code;
    if (0 >
        (code = discord_application_to_jsonb(b, buf, size, this->application)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "interaction",
      ...
      return code;
    if (0 > (code = discord_message_interaction_to_jsonb(b, buf, size,
                                                         this->interaction)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "thread", sizeof("thread") - 1)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "components",
      ...
      return code;
    if (0 >
        (code = discord_components_to_jsonb(b, buf, size, this->components)))
      return code;
...
jsonbcode discord_messages_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_message_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
discord_message_activity_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "party_id", sizeof("party_id") - 1)))
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->party_id,
                                 this->party_id ? strlen(this->party_id) : 0)))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
...
static jsonbcode
discord_reaction_to_jsonb(jsonb *b, char buf[], size_t size,
                          const struct discord_reaction *this) {
  jsonbcode code;
  if (0 > (code = jsonb_object(b, buf, size)))
    return code;
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "me", sizeof("me") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->me)))
      return code;
...
discord_reactions_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_reaction_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
discord_overwrites_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    for (i = 0; i < this->size; ++i)
      if (0 >
          (code = discord_overwrite_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
discord_thread_metadata_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "archived", sizeof("archived") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->archived)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "auto_archive_duration",
                              sizeof("auto_archive_duration") - 1)))
      return code;
    if (0 > (code = jsonb_number(b, buf, size, this->auto_archive_duration)))
      return code;
      ...
      return code;
    {
      ...
      int toklen;
      toklen =
          cog_unix_ms_to_iso8601(tok, sizeof(tok), this->archive_timestamp);
      if (0 > (code = jsonb_string(b, buf, size, tok, toklen)))
        return code;
    };

    if (0 > (code = jsonb_key(b, buf, size, "locked", sizeof("locked") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->locked)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "invitable", sizeof("invitable") - 1)))
      return code;
      ...
      return code;
    {
      char tok[64];
      int toklen;
      toklen = cog_unix_ms_to_iso8601(tok, sizeof(tok), this->create_timestamp);
      if (0 > (code = jsonb_string(b, buf, size, tok, toklen)))
        return code;
    };
  }
...
discord_thread_member_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
      ...
      return code;
    {
      char tok[64];
      int toklen;
      toklen = cog_unix_ms_to_iso8601(tok, sizeof(tok), this->join_timestamp);
      if (0 > (code = jsonb_string(b, buf, size, tok, toklen)))
        return code;
    };
...
discord_attachment_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (this->filename != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "filename", sizeof("filename") - 1)))
        return code;
      if (0 >
          (code = jsonb_string(b, buf, size, this->filename,
    ...
    if (0 > (code = jsonb_key(b, buf, size, "size", sizeof("size") - 1)))
      return code;
    if (0 > (code = jsonb_number(b, buf, size, this->size)))
      return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "proxy_url", sizeof("proxy_url") - 1)))
      return code;
    if (0 >
        (code = jsonb_string(b, buf, size, this->proxy_url,
                             this->proxy_url ? strlen(this->proxy_url) : 0)))
      return code;

    if (this->height != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "height", sizeof("height") - 1)))
        return code;
    ...
    if (this->width != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "width", sizeof("width") - 1)))
        return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "ephemeral", sizeof("ephemeral") - 1)))
      return code;
...
jsonbcode discord_attachments_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    for (i = 0; i < this->size; ++i)
      if (0 >
          (code = discord_attachment_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
jsonbcode discord_embed_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (this->footer != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "footer", sizeof("footer") - 1)))
        return code;
      if (0 >
          (code = discord_embed_footer_to_jsonb(b, buf, size, this->footer)))
        return code;
    }
    ...
    if (this->image != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "image", sizeof("image") - 1)))
        return code;
      if (0 > (code = discord_embed_image_to_jsonb(b, buf, size, this->image)))
        return code;
    }
    ...
    if (this->video != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "video", sizeof("video") - 1)))
        return code;
      if (0 > (code = discord_embed_video_to_jsonb(b, buf, size, this->video)))
        return code;
    }
    if (this->provider != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "provider", sizeof("provider") - 1)))
        return code;
    ...
    if (this->author != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "author", sizeof("author") - 1)))
        return code;
      if (0 >
          (code = discord_embed_author_to_jsonb(b, buf, size, this->author)))
        return code;
    }
    if (this->fields != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "fields", sizeof("fields") - 1)))
        return code;
      if (0 >
          (code = discord_embed_fields_to_jsonb(b, buf, size, this->fields)))
        return code;
    }
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
...
jsonbcode discord_embeds_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_embed_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
discord_embed_author_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (this->icon_url != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "icon_url", sizeof("icon_url") - 1)))
        return code;
      if (0 >
          (code = jsonb_string(b, buf, size, this->icon_url,
    ...
    if (this->proxy_icon_url != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "proxy_icon_url",
        ...
        return code;
      if (0 > (code = jsonb_string(
                   b, buf, size, this->proxy_icon_url,
                   this->proxy_icon_url ? strlen(this->proxy_icon_url) : 0)))
        return code;
    }
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
...
jsonbcode discord_embed_field_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "inline", sizeof("inline") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->Inline)))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
...
discord_embed_fields_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    for (i = 0; i < this->size; ++i)
      if (0 >
          (code = discord_embed_field_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
jsonbcode discord_thread_response_body_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "threads", sizeof("threads") - 1)))
      return code;
    if (0 > (code = discord_channels_to_jsonb(b, buf, size, this->threads)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "members", sizeof("members") - 1)))
      return code;
    if (0 >
        (code = discord_thread_members_to_jsonb(b, buf, size, this->members)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "has_more", sizeof("has_more") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->has_more)))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
  ...
  if (this != NULL) {
    ...
    if (this->video_quality_mode != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "video_quality_mode",
        ...
        return code;
      if (0 > (code = jsonb_number(b, buf, size, this->video_quality_mode)))
        return code;
    }
...
jsonbcode discord_get_channel_messages_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (this->around != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "around", sizeof("around") - 1)))
        return code;
      else {
        char tok[64];
        int toklen;
  ...
  if (this != NULL) {
    ...
    if (this->allowed_mentions != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "allowed_mentions",
        ...
        return code;
      if (0 > (code = discord_allowed_mention_to_jsonb(b, buf, size,
    ...
    if (this->sticker_ids != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "sticker_ids",
        ...
        return code;
      if (0 > (code = snowflakes_to_jsonb(b, buf, size, this->sticker_ids)))
        return code;
    }
...
jsonbcode discord_bulk_delete_messages_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "messages", sizeof("messages") - 1)))
      return code;
...
jsonbcode discord_create_channel_invite_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (this->max_age != 0) {
      if (0 >
          (code = jsonb_key(b, buf, size, "max_age", sizeof("max_age") - 1)))
        return code;
      if (0 > (code = jsonb_number(b, buf, size, this->max_age)))
        return code;
    }
    ...
    if (this->unique != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "unique", sizeof("unique") - 1)))
        return code;
    ...
    if (this->target_application_id != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "target_application_id",
        ...
        char tok[64];
        int toklen;
        toklen = sprintf(tok, "\"%" PRIu64 "\"", this->target_application_id);
        if (0 > (code = jsonb_token(b, buf, size, tok, toklen)))
          return code;
      }
    }
  ...
  if (this != NULL) {
    ...
    if (this->webhook_channel_id != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "webhook_channel_id",
        ...
        char tok[64];
        int toklen;
        toklen = sprintf(tok, "\"%" PRIu64 "\"", this->webhook_channel_id);
        if (0 > (code = jsonb_token(b, buf, size, tok, toklen)))
          return code;
      }
    }
...
jsonbcode discord_group_dm_add_recipient_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "access_token",
      ...
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->access_token,
                                 this->access_token ? strlen(this->access_token)
                                                    : 0)))
      return code;
...
jsonbcode discord_start_thread_with_message_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (this->rate_limit_per_user >= 0 && this->rate_limit_per_user <= 21600) {
      if (0 > (code = jsonb_key(b, buf, size, "rate_limit_per_user",
                                sizeof("rate_limit_per_user") - 1)))
        return code;
      if (0 > (code = jsonb_number(b, buf, size, this->rate_limit_per_user)))
...
jsonbcode discord_emoji_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "animated", sizeof("animated") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->animated)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "available", sizeof("available") - 1)))
      return code;
...
jsonbcode discord_emojis_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_emoji_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
  ...
  if (this != NULL) {
    ...
    if (this->icon_hash != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "icon_hash",
        ...
        return code;
      if (0 >
          (code = jsonb_string(b, buf, size, this->icon_hash,
                               this->icon_hash ? strlen(this->icon_hash) : 0)))
        return code;
    }

    if (0 > (code = jsonb_key(b, buf, size, "splash", sizeof("splash") - 1)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "discovery_splash",
      ...
      return code;
    if (0 > (code = jsonb_string(
                 b, buf, size, this->discovery_splash,
                 this->discovery_splash ? strlen(this->discovery_splash) : 0)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "owner", sizeof("owner") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->owner)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "default_message_notifications",
                              sizeof("default_message_notifications") - 1)))
      return code;
    if (0 > (code = jsonb_number(b, buf, size,
                                 this->default_message_notifications)))
    ...
    if (0 > (code = jsonb_key(b, buf, size, "explicit_content_filter",
                              sizeof("explicit_content_filter") - 1)))
      return code;
    if (0 > (code = jsonb_number(b, buf, size, this->explicit_content_filter)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "emojis", sizeof("emojis") - 1)))
      return code;
    if (0 > (code = discord_emojis_to_jsonb(b, buf, size, this->emojis)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "features", sizeof("features") - 1)))
      return code;
    if (0 > (code = strings_to_jsonb(b, buf, size, this->features)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "mfa_level", sizeof("mfa_level") - 1)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "system_channel_flags",
                              sizeof("system_channel_flags") - 1)))
      return code;
    if (0 > (code = jsonb_number(b, buf, size, this->system_channel_flags)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "large", sizeof("large") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->large)))
      return code;
    ...
    if (this->channels != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "channels", sizeof("channels") - 1)))
        return code;
      if (0 > (code = discord_channels_to_jsonb(b, buf, size, this->channels)))
        return code;
    }
    ...
    if (this->presences != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "presences",
                                sizeof("presences") - 1)))
        return code;
      if (0 > (code = discord_presence_updates_to_jsonb(b, buf, size,
                                                        this->presences)))
        return code;
    }
    ...
    if (0 > (code = jsonb_key(b, buf, size, "banner", sizeof("banner") - 1)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "premium_subscription_count",
      ...
      return code;
    if (0 >
        (code = jsonb_number(b, buf, size, this->premium_subscription_count)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "preferred_locale",
      ...
      return code;
    if (0 > (code = jsonb_string(
                 b, buf, size, this->preferred_locale,
                 this->preferred_locale ? strlen(this->preferred_locale) : 0)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "public_updates_channel_id",
      ...
      char tok[64];
      int toklen;
      toklen = sprintf(tok, "\"%" PRIu64 "\"", this->public_updates_channel_id);
      if (0 > (code = jsonb_token(b, buf, size, tok, toklen)))
        return code;
    }
    ...
    if (this->max_video_channel_users != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "max_video_channel_users",
        ...
        return code;
      if (0 >
          (code = jsonb_number(b, buf, size, this->max_video_channel_users)))
        return code;
    }
    ...
    if (this->approximate_member_count != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "approximate_member_count",
        ...
        return code;
      if (0 >
          (code = jsonb_number(b, buf, size, this->approximate_member_count)))
        return code;
    }
    ...
    if (this->welcome_screen != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "welcome_screen",
        ...
        return code;
      if (0 > (code = discord_welcome_screen_to_jsonb(b, buf, size,
                                                      this->welcome_screen)))
        return code;
    }
    ...
    if (this->stage_instances != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "stage_instances",
        ...
        return code;
      if (0 > (code = discord_stage_instances_to_jsonb(b, buf, size,
    ...
    if (this->stickers != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "stickers", sizeof("stickers") - 1)))
        return code;
      if (0 > (code = discord_stickers_to_jsonb(b, buf, size, this->stickers)))
        return code;
    }
    ...
    if (0 > (code = jsonb_key(b, buf, size, "premium_progress_bar_enabled",
      ...
      return code;
    if (0 >
        (code = jsonb_bool(b, buf, size, this->premium_progress_bar_enabled)))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
...
jsonbcode discord_guilds_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_guild_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
static jsonbcode discord_guild_widget_setting_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "enabled", sizeof("enabled") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->enabled)))
      return code;
...
discord_guild_member_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (this->avatar != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "avatar", sizeof("avatar") - 1)))
        return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "joined_at", sizeof("joined_at") - 1)))
      return code;
    {
      char tok[64];
      int toklen;
    ...
    if (this->premium_since != 0) {
        ...
        return code;
      {
        char tok[64];
        int toklen;
        toklen = cog_unix_ms_to_iso8601(tok, sizeof(tok), this->premium_since);
        if (0 > (code = jsonb_string(b, buf, size, tok, toklen)))
          return code;
      };
    }
    ...
    if (0 > (code = jsonb_key(b, buf, size, "deaf", sizeof("deaf") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->deaf)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "muted", sizeof("muted") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->muted)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "pending", sizeof("pending") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->pending)))
      return code;
...
discord_guild_members_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    for (i = 0; i < this->size; ++i)
      if (0 >
          (code = discord_guild_member_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
discord_integration_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "syncing", sizeof("syncing") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->syncing)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "role_id", sizeof("role_id") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "account", sizeof("account") - 1)))
      return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "synced_at", sizeof("synced_at") - 1)))
      return code;
    {
      char tok[64];
      int toklen;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "revoked", sizeof("revoked") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->revoked)))
      return code;
...
discord_integrations_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    for (i = 0; i < this->size; ++i)
      if (0 >
          (code = discord_integration_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
jsonbcode discord_bans_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_ban_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
static jsonbcode discord_welcome_screen_channel_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "emoji_id", sizeof("emoji_id") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "emoji_name",
      ...
      return code;
    if (0 >
        (code = jsonb_string(b, buf, size, this->emoji_name,
...
static jsonbcode discord_welcome_screen_channels_to_jsonb(
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_welcome_screen_channel_to_jsonb(b, buf, size,
                                                              &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
...
discord_create_guild_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (this->region != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "region", sizeof("region") - 1)))
        return code;
      if (0 > (code = jsonb_string(b, buf, size, this->region,
                                   this->region ? strlen(this->region) : 0)))
        return code;
    }
...
jsonbcode discord_create_guild_channel_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (this->bitrate != 0) {
      if (0 >
          (code = jsonb_key(b, buf, size, "bitrate", sizeof("bitrate") - 1)))
        return code;
      if (0 > (code = jsonb_number(b, buf, size, this->bitrate)))
        return code;
    }
...
discord_add_guild_member_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "mute", sizeof("mute") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->mute)))
      return code;
  ...
  if (this != NULL) {
    ...
    if (this->delete_message_days >= 0 && this->delete_message_days <= 7) {
      if (0 > (code = jsonb_key(b, buf, size, "delete_message_days",
                                sizeof("delete_message_days") - 1)))
        return code;
      if (0 > (code = jsonb_number(b, buf, size, this->delete_message_days)))
        return code;
    }
...
static jsonbcode discord_get_guild_prune_count_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "include_roles",
      ...
      return code;
    if (0 > (code = snowflakes_to_jsonb(b, buf, size, this->include_roles)))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
...
jsonbcode discord_begin_guild_prune_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (this->days != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "days", sizeof("days") - 1)))
        return code;
...
static jsonbcode discord_get_guild_widget_image_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (this->style != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "style", sizeof("style") - 1)))
        return code;
      if (0 > (code = jsonb_string(b, buf, size, this->style,
...
jsonbcode discord_modify_current_user_voice_state_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "suppress", sizeof("suppress") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->suppress)))
      return code;

    if (this->request_to_speak_timestamp != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "request_to_speak_timestamp",
                                sizeof("request_to_speak_timestamp") - 1)))
        return code;
      {
        char tok[64];
...
jsonbcode discord_guild_scheduled_event_to_jsonb(
  ...
  if (this != NULL) {
      ...
      return code;
    {
      ...
      int toklen;
      toklen =
          cog_unix_ms_to_iso8601(tok, sizeof(tok), this->scheduled_start_time);
      if (0 > (code = jsonb_string(b, buf, size, tok, toklen)))
        return code;
    };
      ...
      return code;
    {
      ...
      int toklen;
      toklen =
          cog_unix_ms_to_iso8601(tok, sizeof(tok), this->scheduled_end_time);
      if (0 > (code = jsonb_string(b, buf, size, tok, toklen)))
        return code;
    };
    ...
    if (this->status != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "status", sizeof("status") - 1)))
        return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "entity_id", sizeof("entity_id") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
    ...
    if (this->entity_metadata != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "entity_metadata",
        ...
        return code;
      if (0 > (code = discord_guild_scheduled_event_entity_metadata_to_jsonb(
    ...
    if (this->creator != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "creator", sizeof("creator") - 1)))
        return code;
...
jsonbcode discord_guild_scheduled_events_to_jsonb(
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_guild_scheduled_event_to_jsonb(b, buf, size,
                                                             &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
...
static jsonbcode discord_guild_scheduled_event_entity_metadata_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (this->location != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "location", sizeof("location") - 1)))
        return code;
      if (0 >
          (code = jsonb_string(b, buf, size, this->location,
...
static jsonbcode discord_guild_scheduled_event_user_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "guild_scheduled_event_id",
      ...
      char tok[64];
      int toklen;
      toklen = sprintf(tok, "\"%" PRIu64 "\"", this->guild_scheduled_event_id);
      if (0 > (code = jsonb_token(b, buf, size, tok, toklen)))
        return code;
    }
    ...
    if (this->member != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "member", sizeof("member") - 1)))
        return code;
      if (0 >
          (code = discord_guild_member_to_jsonb(b, buf, size, this->member)))
        return code;
    }
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
...
discord_guild_template_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "creator", sizeof("creator") - 1)))
      return code;
    if (0 > (code = discord_user_to_jsonb(b, buf, size, this->creator)))
      return code;
      ...
      return code;
    {
      char tok[64];
      int toklen;
      toklen = cog_unix_ms_to_iso8601(tok, sizeof(tok), this->updated_at);
      if (0 > (code = jsonb_string(b, buf, size, tok, toklen)))
        return code;
    };
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "is_dirty", sizeof("is_dirty") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->is_dirty)))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
...
jsonbcode discord_sticker_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (this->pack_id != 0) {
      if (0 >
          (code = jsonb_key(b, buf, size, "pack_id", sizeof("pack_id") - 1)))
        return code;
      else {
        char tok[64];
        int toklen;
...
static jsonbcode
discord_stickers_to_jsonb(jsonb *b, char buf[], size_t size,
                          const struct discord_stickers *this) {
  jsonbcode code;
  if (0 > (code = jsonb_array(b, buf, size)))
    return code;
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_sticker_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
discord_sticker_items_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    for (i = 0; i < this->size; ++i)
      if (0 >
          (code = discord_sticker_item_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
discord_sticker_pack_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "sku_id", sizeof("sku_id") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
    ...
    if (this->cover_sticker_id != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "cover_sticker_id",
        ...
        char tok[64];
        int toklen;
        toklen = sprintf(tok, "\"%" PRIu64 "\"", this->cover_sticker_id);
        if (0 > (code = jsonb_token(b, buf, size, tok, toklen)))
          return code;
      }
    }
    ...
    if (this->banner_asset_id != 0) {
      if (0 > (code = jsonb_key(b, buf, size, "banner_asset_id",
        ...
        char tok[64];
        int toklen;
        toklen = sprintf(tok, "\"%" PRIu64 "\"", this->banner_asset_id);
        if (0 > (code = jsonb_token(b, buf, size, tok, toklen)))
          return code;
      }
    }
...
jsonbcode discord_user_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "username", sizeof("username") - 1)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "bot", sizeof("bot") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->bot)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "system", sizeof("system") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->System)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "locale", sizeof("locale") - 1)))
      return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "verified", sizeof("verified") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->verified)))
      return code;
...
jsonbcode discord_users_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_user_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
jsonbcode discord_connections_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    for (i = 0; i < this->size; ++i)
      if (0 >
          (code = discord_connection_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
  ...
  if (this != NULL) {
    ...
    if (this->username != NULL) {
      if (0 >
        ...
        return code;
      if (0 >
          (code = jsonb_string(b, buf, size, this->username,
                               this->username ? strlen(this->username) : 0)))
        return code;
    }
  ...
  if (this != NULL) {
    ...
    if (this->access_tokens != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "access_tokens",
        ...
        return code;
      if (0 > (code = snowflakes_to_jsonb(b, buf, size, this->access_tokens)))
        return code;
    }

    if (this->nicks != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "nicks", sizeof("nicks") - 1)))
        return code;
...
jsonbcode discord_voice_state_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "session_id",
      ...
      return code;
    if (0 >
        (code = jsonb_string(b, buf, size, this->session_id,
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "self_deaf", sizeof("self_deaf") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->self_deaf)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "self_mute", sizeof("self_mute") - 1)))
      return code;
...
discord_voice_states_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    for (i = 0; i < this->size; ++i)
      if (0 >
          (code = discord_voice_state_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
discord_voice_region_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "optimal", sizeof("optimal") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->optimal)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "custom", sizeof("custom") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->custom)))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
...
discord_voice_regions_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    for (i = 0; i < this->size; ++i)
      if (0 >
          (code = discord_voice_region_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
  ...
  if (this != NULL) {
    ...
    if (this->source_channel != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "source_channel",
        ...
        return code;
      if (0 >
          (code = discord_channel_to_jsonb(b, buf, size, this->source_channel)))
        return code;
    }
...
jsonbcode discord_webhooks_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_webhook_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
discord_execute_webhook_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "wait", sizeof("wait") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->wait)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "thread_id", sizeof("thread_id") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "avatar_url",
      ...
      return code;
    if (0 >
        (code = jsonb_string(b, buf, size, this->avatar_url,
...
jsonbcode discord_identify_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "properties",
                              sizeof("properties") - 1)))
      return code;
    if (0 > (code = discord_identify_connection_to_jsonb(b, buf, size,
                                                         this->properties)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "compress", sizeof("compress") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->compress)))
      return code;
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "presence", sizeof("presence") - 1)))
      return code;
    if (0 >
        (code = discord_presence_update_to_jsonb(b, buf, size, this->presence)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "intents", sizeof("intents") - 1)))
      return code;
...
static jsonbcode discord_identify_connection_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "$browser", sizeof("$browser") - 1)))
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->browser,
                                 this->browser ? strlen(this->browser) : 0)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "$device", sizeof("$device") - 1)))
      return code;
...
discord_presence_update_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "activities",
      ...
      return code;
    if (0 >
        (code = discord_activities_to_jsonb(b, buf, size, this->activities)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "afk", sizeof("afk") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->afk)))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
...
static jsonbcode
discord_activity_to_jsonb(jsonb *b, char buf[], size_t size,
                          const struct discord_activity *this) {
  jsonbcode code;
  if (0 > (code = jsonb_object(b, buf, size)))
    return code;
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "timestamps",
                              sizeof("timestamps") - 1)))
      return code;
    if (0 > (code = discord_activity_timestamps_to_jsonb(b, buf, size,
                                                         this->timestamps)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "details", sizeof("details") - 1)))
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->details,
    ...
    if (0 > (code = jsonb_key(b, buf, size, "emoji", sizeof("emoji") - 1)))
      return code;
    if (0 > (code = discord_activity_emoji_to_jsonb(b, buf, size, this->emoji)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "party", sizeof("party") - 1)))
      return code;
    if (0 > (code = discord_activity_party_to_jsonb(b, buf, size, this->party)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "assets", sizeof("assets") - 1)))
      return code;
    if (0 >
        (code = discord_activity_assets_to_jsonb(b, buf, size, this->assets)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "secrets", sizeof("secrets") - 1)))
      return code;
    if (0 >
        (code = discord_activity_secrets_to_jsonb(b, buf, size, this->secrets)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "instance", sizeof("instance") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->instance)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "buttons", sizeof("buttons") - 1)))
      return code;
    if (0 >
        (code = discord_activity_buttons_to_jsonb(b, buf, size, this->buttons)))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
...
discord_activities_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_activity_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
discord_activity_assets_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "large_image",
      ...
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->large_image,
                                 this->large_image ? strlen(this->large_image)
                                                   : 0)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "large_text",
      ...
      return code;
    if (0 >
        (code = jsonb_string(b, buf, size, this->large_text,
    ...
    if (0 > (code = jsonb_key(b, buf, size, "small_image",
      ...
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->small_image,
                                 this->small_image ? strlen(this->small_image)
                                                   : 0)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "small_text",
      ...
      return code;
    if (0 >
        (code = jsonb_string(b, buf, size, this->small_text,
...
discord_activity_secrets_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "spectate", sizeof("spectate") - 1)))
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->spectate,
                                 this->spectate ? strlen(this->spectate) : 0)))
      return code;
...
jsonbcode discord_session_start_limit_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "total", sizeof("total") - 1)))
      return code;
    if (0 > (code = jsonb_number(b, buf, size, this->total)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "remaining", sizeof("remaining") - 1)))
      return code;
  ...
  if (this != NULL) {
    ...
    if (this->members != NULL) {
      if (0 >
        ...
        return code;
      if (0 >
          (code = discord_team_members_to_jsonb(b, buf, size, this->members)))
        return code;
    }
...
discord_team_member_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "team_id", sizeof("team_id") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
...
discord_team_members_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    for (i = 0; i < this->size; ++i)
      if (0 >
          (code = discord_team_member_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
jsonbcode discord_application_command_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "version", sizeof("version") - 1)))
      return code;
    else {
      char tok[64];
      int toklen;
...
jsonbcode discord_application_commands_to_jsonb(
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_application_command_to_jsonb(b, buf, size,
                                                           &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
...
static jsonbcode discord_application_command_option_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (this->required != false) {
      if (0 >
          (code = jsonb_key(b, buf, size, "required", sizeof("required") - 1)))
        return code;
    ...
    if (this->choices != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "choices", sizeof("choices") - 1)))
        return code;
    ...
    if (this->channel_types != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "channel_types",
        ...
        return code;
      if (0 > (code = integers_to_jsonb(b, buf, size, this->channel_types)))
        return code;
    }
    ...
    if (this->min_value != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "min_value",
        ...
        return code;
      if (0 >
          (code = jsonb_string(b, buf, size, this->min_value,
    ...
    if (this->max_value != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "max_value",
        ...
        return code;
      if (0 >
          (code = jsonb_string(b, buf, size, this->max_value,
...
static jsonbcode discord_application_command_option_choice_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "value", sizeof("value") - 1)))
      return code;
    if (0 >
        (code = jsonb_token(b, buf, size, this->value, strlen(this->value))))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
  ...
  if (this != NULL) {
    ...
    if (this->options != NULL) {
      if (0 >
        ...
        return code;
      if (0 >
          (code = discord_application_command_interaction_data_options_to_jsonb(
               b, buf, size, this->options)))
        return code;
    }

    if (0 > (code = jsonb_key(b, buf, size, "focused", sizeof("focused") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->focused)))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
...
static jsonbcode discord_application_command_interaction_data_options_to_jsonb(
  ...
  if (this != NULL) {
    ...
    for (i = 0; i < this->size; ++i)
      if (0 >
          (code = discord_application_command_interaction_data_option_to_jsonb(
               b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
...
jsonbcode discord_guild_application_command_permissions_to_jsonb(
  ...
  if (this != NULL) {
    int i;
    for (i = 0; i < this->size; ++i)
      if (0 > (code = discord_guild_application_command_permission_to_jsonb(
                   b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
...
jsonbcode discord_component_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "custom_id", sizeof("custom_id") - 1)))
      return code;
    if (0 >
        (code = jsonb_string(b, buf, size, this->custom_id,
                             this->custom_id ? strlen(this->custom_id) : 0)))
      return code;

    if (0 >
        (code = jsonb_key(b, buf, size, "disabled", sizeof("disabled") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->disabled)))
      return code;
    ...
    if (this->options != NULL) {
      if (0 >
        ...
        return code;
      if (0 >
          (code = discord_select_options_to_jsonb(b, buf, size, this->options)))
        return code;
    }
    ...
    if (0 > (code = jsonb_key(b, buf, size, "placeholder",
      ...
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->placeholder,
                                 this->placeholder ? strlen(this->placeholder)
                                                   : 0)))
      return code;
...
jsonbcode discord_components_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    for (i = 0; i < this->size; ++i)
      if (0 >
          (code = discord_component_to_jsonb(b, buf, size, &this->array[i])))
        return code;
  }
  if (0 > (code = jsonb_array_pop(b, buf, size)))
    return code;
...
discord_select_option_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "default", sizeof("default") - 1)))
      return code;
    if (0 > (code = jsonb_bool(b, buf, size, this->Default)))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
    return code;
...
jsonbcode discord_interaction_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 > (code = jsonb_key(b, buf, size, "data", sizeof("data") - 1)))
      return code;
    if (0 >
        (code = discord_interaction_data_to_jsonb(b, buf, size, this->data)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "message", sizeof("message") - 1)))
      return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "guild_locale",
      ...
      return code;
    if (0 > (code = jsonb_string(b, buf, size, this->guild_locale,
                                 this->guild_locale ? strlen(this->guild_locale)
                                                    : 0)))
      return code;
  }
  if (0 > (code = jsonb_object_pop(b, buf, size)))
...
discord_interaction_data_to_jsonb(jsonb *b, char buf[], size_t size,
  ...
  if (this != NULL) {
    ...
    if (0 >
        (code = jsonb_key(b, buf, size, "resolved", sizeof("resolved") - 1)))
      return code;
    if (0 >
        (code = discord_resolved_data_to_jsonb(b, buf, size, this->resolved)))
      return code;

    if (0 > (code = jsonb_key(b, buf, size, "options", sizeof("options") - 1)))
      return code;
    if (0 >
        (code = discord_application_command_interaction_data_options_to_jsonb(
             b, buf, size, this->options)))
    ...
    if (0 > (code = jsonb_key(b, buf, size, "values", sizeof("values") - 1)))
      return code;
...
jsonbcode discord_interaction_response_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (this->data != NULL) {
      if (0 > (code = jsonb_key(b, buf, size, "data", sizeof("data") - 1)))
        return code;
      if (0 > (code = discord_interaction_callback_data_to_jsonb(b, buf, size,
                                                                 this->data)))
        return code;
    }
  }
...
static jsonbcode discord_interaction_callback_data_to_jsonb(
  ...
  if (this != NULL) {
    ...
    if (this->content != NULL) {
      if (0 >
          (code = jsonb_key(b, buf, size, "content", sizeof("content") - 1)))
        return code;
      if (0 > (code = jsonb_string(b, buf, size, this->content,
                                   this->content ? strlen(this->content) : 0)))
        return code;
    ...
    if (0 > (code = jsonb_key(b, buf, size, "choices", sizeof("choices") - 1)))
      return code;
    if (0 > (code = discord_application_command_option_choices_to_jsonb(
                 b, buf, size, this->choices)))
      return code;
...
size_t discord_modify_guild_channel_positions_to_json(
  ...
  jsonbcode code;
  jsonb_init(&b);
  code = discord_modify_guild_channel_positions_to_jsonb(&b, buf, size, this);
  return code < 0 ? 0 : b.pos;
}
...
size_t discord_create_guild_role_to_json(
    char buf[], size_t size, const struct discord_create_guild_role *this) {
  jsonb b;
  jsonbcode code;
  jsonb_init(&b);
...
size_t discord_modify_guild_role_to_json(
    char buf[], size_t size, const struct discord_modify_guild_role *this) {
  jsonb b;
  jsonbcode code;
  jsonb_init(&b);
...
size_t discord_begin_guild_prune_to_json(
    char buf[], size_t size, const struct discord_begin_guild_prune *this) {
  jsonb b;
  jsonbcode code;
  jsonb_init(&b);
...
size_t discord_modify_current_user_voice_state_to_json(
  ...
  jsonbcode code;
  jsonb_init(&b);
  code = discord_modify_current_user_voice_state_to_jsonb(&b, buf, size, this);
  return code < 0 ? 0 : b.pos;
}
...
size_t discord_list_scheduled_events_for_guild_to_json(
  ...
  jsonbcode code;
  jsonb_init(&b);
  code = discord_list_scheduled_events_for_guild_to_jsonb(&b, buf, size, this);
  return code < 0 ? 0 : b.pos;
}
...
size_t discord_get_guild_scheduled_event_users_to_json(
  ...
  jsonbcode code;
  jsonb_init(&b);
  code = discord_get_guild_scheduled_event_users_to_jsonb(&b, buf, size, this);
  return code < 0 ? 0 : b.pos;
}
...
size_t discord_create_guild_from_guild_template_to_json(
  ...
  jsonbcode code;
  jsonb_init(&b);
  code = discord_create_guild_from_guild_template_to_jsonb(&b, buf, size, this);
  return code < 0 ? 0 : b.pos;
}
...
size_t discord_guild_application_command_permissions_to_json(
  ...
  jsonbcode code;
  jsonb_init(&b);
  code = discord_guild_application_command_permissions_to_jsonb(&b, buf, size,
                                                                this);
  return code < 0 ? 0 : b.pos;
}
...
size_t discord_application_command_permission_to_json(
  ...
  jsonbcode code;
  jsonb_init(&b);
  code = discord_application_command_permission_to_jsonb(&b, buf, size, this);
  return code < 0 ? 0 : b.pos;
}
...
size_t discord_application_command_permissions_to_json(
  ...
  jsonbcode code;
  jsonb_init(&b);
  code = discord_application_command_permissions_to_jsonb(&b, buf, size, this);
  return code < 0 ? 0 : b.pos;
}
...
size_t discord_create_global_application_command_to_json(
  ...
  jsonb_init(&b);
  code =
      discord_create_global_application_command_to_jsonb(&b, buf, size, this);
  return code < 0 ? 0 : b.pos;
}
...
size_t discord_edit_global_application_command_to_json(
  ...
  jsonbcode code;
  jsonb_init(&b);
  code = discord_edit_global_application_command_to_jsonb(&b, buf, size, this);
  return code < 0 ? 0 : b.pos;
}
...
size_t discord_create_guild_application_command_to_json(
  ...
  jsonbcode code;
  jsonb_init(&b);
  code = discord_create_guild_application_command_to_jsonb(&b, buf, size, this);
  return code < 0 ? 0 : b.pos;
}
...
size_t discord_edit_guild_application_command_to_json(
  ...
  jsonbcode code;
  jsonb_init(&b);
  code = discord_edit_guild_application_command_to_jsonb(&b, buf, size, this);
  return code < 0 ? 0 : b.pos;
}
...
size_t discord_edit_original_interaction_response_to_json(
  ...
  jsonb_init(&b);
  code =
      discord_edit_original_interaction_response_to_jsonb(&b, buf, size, this);
  return code < 0 ? 0 : b.pos;
}
